...
class GUI:
    ...
    def search_member_menu(self):
        ...
        back_button = tk.Button(self.root, text="Kembali", command=self.user_menu)
        back_button.pack()

    def search_member(self):
        required_skills = self.skillset_entry.get().split(',')
        required_skills = [skill.strip() for skill in required_skills]

        members = self.team.find_members(required_skills)
        if members:
            result = "\n".join([f"{member.name}: {', '.join(member.skillset)}" for member in members])
            messagebox.showinfo("Hasil Pencarian", result)
        else:
            messagebox.showinfo("Hasil Pencarian", "Tidak ada anggota yang cocok dengan kemampuan yang dicari.")
